...
from azure.cosmos import PartitionKey  # Importing PartitionKey to define how data is partitioned
...
@bp.before_request
async def capture_request_data():
    # Capture request data
    request_data = await request.get_json()

    # Store request data in Cosmos DB
    cosmos_container = current_app.config["COSMOS_CONTAINER"]
    request_record = {
        "id": str(uuid.uuid4()), 
        "source": "api_request",
        "userId": 0, 
    ...
    await cosmos_container.create_item(body=request_record)

    # Save request ID to g for later use
    g.request_id = request_record['id']

@bp.after_request
async def capture_response_data(response):
    # Capture response data
    response_data = await response.get_json() if response.is_json else None
    ...
    response_record = {
        ...
        "request_id": g.get('request_id'),  # Link to the request record
        "timestamp": time.time(),
    }
    await cosmos_container.create_item(body=response_record)
...
async def chat_stream(auth_claims: Dict[str, Any]):
    current_app.logger.info(f"App Insights Connection String: {os.getenv('APPLICATIONINSIGHTS_CONNECTION_STRING')}")
    ...
    try:
        ...
        dummy_data = {
            "id": str(uuid.uuid4()),  # Ensure this ID is unique or change it each time
            "source" : "sales",
            "userId": 0,  # Example: use user's OID as partition key
            "query": request_json["messages"][-1]["content"],  # Save the incoming messages
            "response":  "test",  # Placeholder response
            "timestamp": "Dummy",  # Save the current timestamp
        }
        await cosmos_container.create_item(body=dummy_data)
...
async def setup_clients():
    ...
    # Set up Cosmos DB 
    COSMOS_DB_ENDPOINT = "https://salescosmo.documents.azure.com:443/" #should be moved as env variables
    COSMOS_DB_KEY = "hkEhlbVsl8dl0DbltH82PCy5V8wtpDlfal9k4mGO9mKWXuOFETpswMmWehaC86mGHrp5DhpgpW1KACDb1zpEVg=="
    COSMOS_DB_DATABASE_NAME = "stcbrain"
    COSMOS_DB_CONTAINER_NAME = "UserInteractions"
    cosmos_client = CosmosClient(COSMOS_DB_ENDPOINT, credential=COSMOS_DB_KEY)
    database = cosmos_client.get_database_client(COSMOS_DB_DATABASE_NAME)  # Directly get the existing database
    container = database.get_container_client(COSMOS_DB_CONTAINER_NAME)  # Directly get the existing container
    current_app.config["COSMOS_CONTAINER"] = container
